...
function getRoomId(){
    ...
    fetch('/register',{method:'post',body:JSON.stringify(data),headers})
    .then(Response=>
        Response.json()
        ).then(result=>{
...
function sendVideo(){
   ...
   data1=document.getElementById('docfile').files[0];
   const formData = new FormData();
  ...
  //send audio on socket server
   function sendAudio(){
   data1=document.getElementById('audiofile').files[0];
   const formData = new FormData();
  ...
  //send camera  images and video
   function sendCamera(){
   data1=document.getElementById('camera').files[0];
   const formData = new FormData();
